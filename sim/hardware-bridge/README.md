# Local hardware bridge server (Phase 2)
...
- Node.js ≥ 20
- Delay (or other) module firmware flashed with USB neighbor support
...
- **JSON control** (text WebSocket frames): `ping`, `session` start/stop, `enumerate`, `status` responses
- **Binary relay** (binary WebSocket frames): length-prefixed Phase 1 USB wire frames — four mono int16 paths at 128 samples, 22.05 kHz
...
| Underrun badge | USB latency; ring depth absorbs ~2 periods — see simulator docs |
