...
inline void bridgeHandleUsbWirePayload(const uint8_t* payload, size_t payloadLen) {
  ...
  switch (command) {
    ...
    case BRIDGE_CMD_EXCHANGE_USB_PACK4: {
      if (!g_usb_neighbor_state.active) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_PACK4, 0, BRIDGE_STATUS_ERR_NOT_ACTIVE);
        return;
      }
      if (payloadLen < BRIDGE_PACK4_REQUEST_SIZE) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_PACK4, 0, BRIDGE_STATUS_ERR_SHORT_READ);
        return;
      }

      uint32_t baseSequence = 0;
      memcpy(&baseSequence, payload + BRIDGE_HEADER_SIZE, sizeof(baseSequence));
      memcpy(&g_usb_neighbor_state.primaryControl,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_PACK4_AUDIO_BYTES * 2,
             sizeof(float));
      memcpy(&g_usb_neighbor_state.secondaryControl,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_PACK4_AUDIO_BYTES * 2 + sizeof(float),
             sizeof(float));

      static int16_t pack4DownstreamIn[BUFFER_LEN * BRIDGE_USB_PACK_PERIODS];
      static int16_t pack4UpstreamIn[BUFFER_LEN * BRIDGE_USB_PACK_PERIODS];
      static int16_t pack4DownstreamOut[BUFFER_LEN * BRIDGE_USB_PACK_PERIODS];
      static int16_t pack4UpstreamOut[BUFFER_LEN * BRIDGE_USB_PACK_PERIODS];
      memcpy(pack4DownstreamIn,
             payload + BRIDGE_HEADER_SIZE + 4,
             BRIDGE_PACK4_AUDIO_BYTES);
      memcpy(pack4UpstreamIn,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_PACK4_AUDIO_BYTES,
             BRIDGE_PACK4_AUDIO_BYTES);

      const uint16_t status = usbNeighborProcessPack4Exchange(
        g_usb_neighbor_state,
        baseSequence,
        pack4DownstreamIn,
        pack4UpstreamIn,
        pack4DownstreamOut,
      ...
      if (status != BRIDGE_STATUS_OK) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_PACK4, baseSequence, status);
        return;
      }

      static uint8_t frame[BRIDGE_PACK4_RESPONSE_SIZE];
      bridgeWriteHeader(frame, BRIDGE_CMD_EXCHANGE_USB_PACK4);
      size_t offset = BRIDGE_HEADER_SIZE;
      const uint32_t lastSequence = baseSequence + BRIDGE_USB_PACK_PERIODS - 1u;
      memcpy(frame + offset, &lastSequence, sizeof(lastSequence));
      offset += sizeof(lastSequence);
      memcpy(frame + offset, pack4DownstreamOut, BRIDGE_PACK4_AUDIO_BYTES);
      offset += BRIDGE_PACK4_AUDIO_BYTES;
      memcpy(frame + offset, pack4UpstreamOut, BRIDGE_PACK4_AUDIO_BYTES);
      offset += BRIDGE_PACK4_AUDIO_BYTES;
      const uint16_t okStatus = BRIDGE_STATUS_OK;
      memcpy(frame + offset, &okStatus, sizeof(okStatus));
      offset += sizeof(okStatus);
      const uint32_t timestampUs = (uint32_t)((uint64_t)esp_timer_get_time() - g_usb_neighbor_state.streamStartUs);
      memcpy(frame + offset, &timestampUs, sizeof(timestampUs));
      bridgeWriteLengthPrefixed(bridgeAudioStream(), frame, sizeof(frame));
      break;
    }

    case BRIDGE_CMD_EXCHANGE_USB_22K_MONO: {
      if (!g_usb_neighbor_state.active) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_22K_MONO, 0, BRIDGE_STATUS_ERR_NOT_ACTIVE);
        return;
      }
      if (payloadLen < BRIDGE_SPIKE_MONO_REQUEST_SIZE) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_22K_MONO, 0, BRIDGE_STATUS_ERR_SHORT_READ);
        return;
      }

      uint32_t sequence = 0;
      memcpy(&sequence, payload + BRIDGE_HEADER_SIZE, sizeof(sequence));
      memcpy(&g_usb_neighbor_state.primaryControl,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_SPIKE_MONO_AUDIO_BYTES * 2,
             sizeof(float));
      memcpy(&g_usb_neighbor_state.secondaryControl,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_SPIKE_MONO_AUDIO_BYTES * 2 + sizeof(float),
             sizeof(float));

      static int16_t monoDownstreamIn[BRIDGE_SPIKE_MONO_BUFFER_LEN];
      static int16_t monoUpstreamIn[BRIDGE_SPIKE_MONO_BUFFER_LEN];
      static int16_t monoDownstreamOut[BRIDGE_SPIKE_MONO_BUFFER_LEN];
      static int16_t monoUpstreamOut[BRIDGE_SPIKE_MONO_BUFFER_LEN];
      memcpy(monoDownstreamIn, payload + BRIDGE_HEADER_SIZE + 4, BRIDGE_SPIKE_MONO_AUDIO_BYTES);
      memcpy(monoUpstreamIn,
             payload + BRIDGE_HEADER_SIZE + 4 + BRIDGE_SPIKE_MONO_AUDIO_BYTES,
             BRIDGE_SPIKE_MONO_AUDIO_BYTES);

      const uint16_t status = usbNeighborProcess22kMonoExchange(
        g_usb_neighbor_state,
        sequence,
        monoDownstreamIn,
        monoUpstreamIn,
        ...
        monoUpstreamOut);
      if (status != BRIDGE_STATUS_OK) {
        bridgeWriteUsbAck(BRIDGE_CMD_EXCHANGE_USB_22K_MONO, sequence, status);
        return;
      }

      static uint8_t frame[BRIDGE_SPIKE_MONO_RESPONSE_SIZE];
      bridgeWriteHeader(frame, BRIDGE_CMD_EXCHANGE_USB_22K_MONO);
      size_t offset = BRIDGE_HEADER_SIZE;
      memcpy(frame + offset, &sequence, sizeof(sequence));
      offset += sizeof(sequence);
      memcpy(frame + offset, monoDownstreamOut, BRIDGE_SPIKE_MONO_AUDIO_BYTES);
      offset += BRIDGE_SPIKE_MONO_AUDIO_BYTES;
      memcpy(frame + offset, monoUpstreamOut, BRIDGE_SPIKE_MONO_AUDIO_BYTES);
      offset += BRIDGE_SPIKE_MONO_AUDIO_BYTES;
      const uint16_t okStatus = BRIDGE_STATUS_OK;
      memcpy(frame + offset, &okStatus, sizeof(okStatus));
      offset += sizeof(okStatus);
