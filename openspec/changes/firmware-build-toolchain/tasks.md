...
- [x] 2.1 Add `scripts/build.sh` accepting module name (delay, merger, cutoff, debug_tone, passthrough) and optional `all` to compile each variant
- [x] 2.2 Add `scripts/upload.sh` resolving serial port from `.env` or sole connected device, then flashing after compile
- [x] 2.3 Add `scripts/monitor.sh` (or equivalent) capturing serial output for a bounded duration after upload

## 3. Documentation

- [x] 3.1 Add README "Build and flash" section: prerequisites (arduino-cli, esp32 core, dialout), build/upload/monitor usage, BOOT-button note for S3-Zero
- [x] 3.2 Update `AGENTS.md` guide index to reference README build section for compile/upload verification
